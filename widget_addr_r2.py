...
class WidgetAddr(QWidget):
    ...
    def __init__(self, parent, no=0, seq=0, title=u'새주소록', public=False, numcols=0):
        ...
        self.menuColumn = QMenu()
        self.ui.toolColumns.setMenu(self.menuColumn)
...
#        os.path.splitext(fullFileName)[0]
    ...
    @Slot()
    def on_toolCheckableTree_clicked(self):
        print 'on_toolCheckableTree_clicked() called'
        isChecked = self.ui.toolCheckableTree.isChecked()
        self.ui.treeAddr.setVisible(isChecked)
    
    @Slot()
    def on_toolColumns_clicked(self):
        print 'on_toolColumns_clicked() called!'
        self.ui.toolColumns.showMenu()
          
    @Slot(QAction)
    def on_toolColumns_triggered(self, action):
        print 'on_toolColumns_triggered() called!'
        print action.isChecked()

    @Slot(QAction)
    def on_toolCheckableTree_triggered(self, action):
        print 'on_toolCheckableTree_triggered() called!'
        print action
    ...
    def getAddrLabelsFromDB(self):
        query_head = 'select '
        query_body = ''
        query_tail = ' from addr_labels_r2 where no_addr_list=%d' % self._addrNo
...
#        print 'getAddrLabelsFromDB() => query:', query
    ...
    def setAddrLabelsToTable(self):
        # count labels that can be show.
        self._labelsToShow = [] 
        for col in self._rowsCols:
            dict = eval(col)
            show = dict['show']
            title = dict['title'].decode('utf8')
            action = self.menuColumn.addAction(title)
            action.setCheckable(True)
            if show == 1:
                self._labelsToShow.append(title)
...
#        self.ui.tableAddr.setColumnCount(self._numCols - 3)
        self.ui.tableAddr.setColumnCount(len(self._labelsToShow))
        self.ui.tableAddr.setHorizontalHeaderLabels(self._labelsToShow)
    ...
    def checkDataValidation(self):
        ...
        if self._fileExt == "txt":
            ...
            try:
                ...
                idx = 0
                for value in labels:
                    labels[idx] = u"{'title':'%s','show':1}" % value
                    print labels[idx]
                    idx += 1
                self._labels = labels
#                print 'labels:', labels
...
#                traceback.print_exc() # need to import module "traceback"
        else:
            return False
    ...
    def insertAddrLabelIntoDB(self):
        ...
        query_full = "insert into addr_labels_r2 (%s) values (%s)" % (query_sub1, query_sub2)
#        print query_full
